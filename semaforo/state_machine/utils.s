...
AUTO100000000 .field 100000000
AUTO0x01850185 .field 0x01850185
AUTO80000000 .field 80000000
AUTO0x01440144 .field 0x01440144
AUTO60000000 .field 60000000
AUTO0x01030103 .field 0x01030103
AUTO40000000 .field 40000000
AUTO16000000 .field 16000000
; -------------------------------------------------------------------------------------------------------------------------
; PLL
; -------------------------------------------------------------------------------------------------------------------------
...
SYSCLK .field (FXTAL/(Q+1)/(N+1))*(MINT+MFRAC/1024)/(PSYSDIV+1)

DELAY1MS .field 80000 ; n�mero de ciclos de clock para contar 1ms (assumindo 80 MHz)
...
PLL_Init_step7fullspeed                        ; 100 MHz < SysClk <= 120 MHz: "full speed"
    LDR R3, AUTO100000000                         ; R3 = 100,000,000 (valor)
    CMP R2, R3                                 ; � R2 (SysClk) <= R3 (100,000,000 Hz)?
    BLS PLL_Init_step7veryfast                 ; se sim, pula o pr�ximo teste
    LDR R3, AUTO0x01850185                        ; R3 = 0x01850185 (valores deslocados)
...
PLL_Init_step7slow                             ; 16 MHz < SysClk <= 40 MHz: "slow"
    LDR R3, AUTO16000000                          ; R3 = 16,000,000 (value)
...
PLL_Init_step8done
    ...
    LDR R1, SYSCTL_RSCLKCFG_R                 ; R1 = SYSCTL_RSCLKCFG_R (ponteiro)
    LDR R0, [R1]                               ; R0 = [R1] (valor)
    MOV R3, #SYSCTL_RSCLKCFG_PSYSDIV_M         ; R3 = SYSCTL_RSCLKCFG_PSYSDIV_M (mascara)
    ...
    ;ADD R0, R0, #PSYSDIV  ; R0 = R0 + (PSYSDIV&SYSCTL_RSCLKCFG_PSYSDIV_M) (configurar PSYSDIV como definido acima)
...
SysTick_Init
	...
	MOVW R0, #0xFFFF; 			; valor m�ximo de recarga 2^24 ticks
	MOVT R0, #0x00FF
